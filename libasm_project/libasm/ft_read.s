...
_ft_read:
    ...
    ; SYSCALL
    mov rax, SYS_READ  ; for write syscall
 ...
 .PROC_CHECK_ERROR___FREEBSD:
    jc _ft_read.PROC_SET_ERRNO ; if CF not set, then jump to end
    jmp _ft_read.PROC_END

 .PROC_CHECK_ERROR___LINUX:
    cmp rax, 0 ;
    jl _ft_read.PROC_NEGATE_RAX___LINUX
    jmp _ft_read.PROC_END

 .PROC_NEGATE_RAX___LINUX:
    neg rax ; -N : N (2's compliment)
    jmp _ft_read.PROC_SET_ERRNO
